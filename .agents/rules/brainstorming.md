...
4. **Visual Workspace Sync (Stitch)**: You MUST synchronize the drafted design spec with Stitch in an isolated project for the current topic:
   a. Identify the `<topic-name>` of the current brainstorming session (e.g., "fleet-redesign", "services-showcase").
...
2. Interview the user relentlessly about the drafted design spec, asking questions one at a time.
   ...
   - **For UI/UX**: Grill layout details, responsive breakpoints, and animations. Modify/refine the visual mockups dynamically in Stitch using the **`stitch`** MCP (`edit_screens`, `generate_variants`) with the resolved `projectId` of the current topic.

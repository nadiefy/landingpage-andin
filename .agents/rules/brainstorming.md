...
4. **Visual Workspace Sync**: Once the spec is drafted, upload it using the **`stitch`** MCP (`upload_design_md` and `create_design_system_from_design_md`). List existing screens (`list_screens`) and generate variants (`generate_variants`) to sync the visual mockup space with the design spec.
...
2. Interview the user relentlessly about the drafted design spec, asking questions one at a time.
   ...
   - **For UI/UX**: Grill layout details, responsive breakpoints, and animations. Modify/refine the visual mockups dynamically in Stitch using the **`stitch`** MCP (`edit_screens`, `generate_variants`).
...
4. **Final Sync**: For UI/UX designs, sync the final agreed-upon spec back to Stitch using `stitch:update_design_system`.

...
Before taking action, read `references/explore-domains.md` and use it to classify the request into its primary domain. Load the listed Primary skill as the main lens, then add 1-2 Supporting skills when they add context. Use Verify skills before completion. For multi-domain requests, follow the routing guidance in that reference.

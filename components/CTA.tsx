      ...
      >
        ...
        <div className="relative z-10 p-8 md:p-16 lg:p-20 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <motion.div 
            ...
            className="relative w-full h-[400px] lg:h-[600px] rounded-3xl overflow-hidden border border-white/10 group"
          >
            <iframe 
              ...
              width="100%" 
              height="100%" 
              style={{ border: 0, filter: 'invert(90%) hue-rotate(180deg) contrast(85%) opacity(0.8)' }} 
              allowFullScreen 
              loading="lazy"
              referrerPolicy="no-referrer-when-downgrade"
              className="transition-all duration-500 group-hover:filter-[invert(90%)_hue-rotate(180deg)_contrast(100%)_opacity(1)]"
            ...
            <a 
              ...
              target="_blank" 
              rel="noopener noreferrer"
              className="absolute bottom-6 left-6 right-6 bg-black/80 backdrop-blur-md border border-white/10 text-white py-4 px-6 rounded-2xl flex items-center justify-between hover:bg-black transition-colors"
            >
              <span className="font-medium">Open in Google Maps</span>
              <ArrowUpRight className="w-5 h-5 text-accent-warm" />
            </a>
          </motion.div>
        </div>
      </motion.div>
